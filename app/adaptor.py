...
def _customer(row: dict) -> dict:
    ...
    return {
        ...
        "nome_fantasia": row.get("nickname"),
        "cpf": row.get("cpf"),
        "cnpj": row.get("cnpj"),
        "rg": row.get("rg"),
        "inscricao_estadual": row.get("state_inscription"),
        "tipo": row.get("type"),
        "data_nascimento": row.get("birth_date"),
        "genero": row.get("gender"),
        "endereco": row.get("address"),
        "numero": row.get("number"),
        "complemento": row.get("complement"),
        "bairro": row.get("neighborhood"),
        "cidade": row.get("city"),
        "estado": row.get("state"),
        "cep": row.get("zip_code"),
        "emails": [{"email": email, "principal": True}] if email else [],
        "celular": row.get("cellphone"),
        "telefone": row.get("phone"),
        "enderecos": addresses,
        "total_pedidos": row.get("total_orders"),
        "observacao": row.get("observation"),
        "revendedor": row.get("reseller"),
        "desconto": row.get("discount"),
        "limite_credito": row.get("credit_limit"),
        "indicador_id": row.get("indicator_id"),
        "perfil_cliente_id": row.get("profile_customer_id"),
        "ultima_compra": row.get("last_purchase"),
        "ultima_visita": row.get("last_visit"),
